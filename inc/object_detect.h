...
public:
    ...
    Result Init();
    Result Init(std::string rtsp_url, aclvdecCallback func, aclvencCallback encFunc);
    Result Preprocess(ImageData& resizedImage, ImageData& srcImage);
    Result Preprocess_Video(ImageData& resizedImage, ImageData& srcImage);
    Result Inference(aclmdlDataset*& inferenceOutput, ImageData& resizedImage);
    Result InferenceAsync(aclmdlDataset*& inferenceOutput, ImageData& resizedImage);
    Result Postprocess(ImageData& image, aclmdlDataset* modelOutput,
                       const string& origImagePath);
    Result Postprocess_Video(ImageData& image, aclmdlDataset* modelOutput);
    Result SendVencFrame(acldvppPicDesc *encodeInputDesc_);
    Result SendVencFrameEnd();
    void Wait();
private:
    Result InitResource();
