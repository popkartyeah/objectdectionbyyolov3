...
Result ObjectDetect::Init(std::string rtsp_url, aclvdecCallback func, aclvencCallback encFunc) {
    if (isInited_) {
        INFO_LOG("Classify instance is initied already!");
        return SUCCESS;
    }
    ...
    ret = dvenc_.InitResource();
    if (ret != SUCCESS) {
        ERROR_LOG("Init venc failed");
        return FAILED;
    }
    ret = dvpp_.Restrister(rtsp_url, func);
    if (ret != SUCCESS) {
        ERROR_LOG("Init dvpp failed");
        return FAILED;
    }

    ret = dvenc_.Restrister(encFunc);
    if (ret != SUCCESS) {
        ERROR_LOG("Init venc failed");
        return FAILED;
    }
...
Result ObjectDetect::Preprocess(ImageData& resizedImage, ImageData& srcImage) {
    ...
    INFO_LOG("copy image time %ld us", (end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec));
    ...
    gettimeofday(&end, NULL);
    INFO_LOG("decode jpg %ld us", (end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec));
    ...
    gettimeofday(&end, NULL);
    INFO_LOG("resize jpg %ld us", (end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec));

    return SUCCESS;
}

Result ObjectDetect::Preprocess_Video(ImageData& resizedImage, ImageData& srcImage) {
    struct timeval start, end;
//    gettimeofday(&start, NULL);
//    ImageData imageDevice;
//    Utils::CopyImageDataToDevice(imageDevice, srcImage, runMode_);
...
Result ObjectDetect::InferenceAsync(aclmdlDataset*& inferenceOutput,
ImageData& resizedImage) {
    aclrtSetCurrentContext(context_);
    Result ret = model_.CreateInput(resizedImage.data.get(),
    resizedImage.size,
    ...
    ret = model_.ExecuteAsync(stream_);
    if (ret != SUCCESS) {
        ERROR_LOG("Execute model inference failed");
        return FAILED;
    }
...
Result ObjectDetect::Postprocess_Video(ImageData& image, aclmdlDataset* modelOutput) {
    uint32_t dataSize = 0;
    float* detectData = (float *)GetInferenceOutputItem(dataSize, modelOutput,
    kBBoxDataBufId);
    ...
    for (uint32_t i = 0; i < totalBox; i++) {
        ...
        INFO_LOG("%d %d %d %d %s\n", boundBox.rect.ltX, boundBox.rect.ltY,
        boundBox.rect.rbX, boundBox.rect.rbY, boundBox.text.c_str());
        drawRects.push_back(boundBox.rect);
        detectResults.emplace_back(boundBox);
    }

    dvpp_.DrawRect(image, drawRects);
//    Utils::WriteImageFile(image, "/home/HwHiAiUser/test_infference.nv12");
//    DrowBoundBoxToImage(detectResults, origImagePath);
    if (runMode_ == ACL_HOST) {
        delete[]((uint8_t *)detectData);
        delete[]((uint8_t*)boxNum);
...
void ObjectDetect::Wait()
{
    dvpp_.Wait();
}

Result ObjectDetect::SendVencFrame(acldvppPicDesc *encodeInputDesc_)
{
    return dvenc_.SendFrame(encodeInputDesc_);
}

Result ObjectDetect::SendVencFrameEnd()
{
    return dvenc_.SendFrameEnd();
}