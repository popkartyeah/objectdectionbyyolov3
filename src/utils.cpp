...
int Utils::WriteImageFile(ImageData& image, std::string fileName)
{
    std::ofstream binFile(fileName, std::ofstream::binary | std::ofstream::trunc);
    if (binFile.is_open() == false) {
        ERROR_LOG("open file %s failed", fileName.c_str());
        return FAILED;
    }
    uint32_t binFileBufferLen = YUV420SP_SIZE(image.alignWidth, image.alignHeight);
    INFO_LOG("write w:%d , h:%d aw:%d ah:%d", image.width, image.height, image.alignWidth, image.alignHeight);
    binFile.write((char *)image.data.get(), binFileBufferLen);
    binFile.flush();
    binFile.close();
    return SUCCESS;
}
